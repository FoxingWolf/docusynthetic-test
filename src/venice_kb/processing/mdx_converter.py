...
def _convert_notes_warnings(content: str) -> str:
    ...
    # Note
    content = re.sub(r"<Note>(.*?)</Note>", r"> **Note:** \1", content, flags=re.DOTALL)

    # Warning
    content = re.sub(r"<Warning>(.*?)</Warning>", r"> ⚠️ **Warning:** \1", content, flags=re.DOTALL)
...
def _convert_accordion(content: str) -> str:
    ...
    content = re.sub(
        pattern, r"<details><summary>\1</summary>\n\2\n</details>", content, flags=re.DOTALL
    )
